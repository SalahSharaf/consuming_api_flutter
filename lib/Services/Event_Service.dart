...
class Events_Service {
  ...
  Future<ApiResponse<bool>> createEvent(EventPost item) {
    return http.post(API + "/notes", headers: APIKEYHeaders,body: json.encode(item.toJson())).then((value) {
      if (value.statusCode == 201) {
        return ApiResponse<bool>(true, false, "");
      } else {
        return ApiResponse<bool>(null, true, "error occurred");
      }
    }).catchError((onError) => ApiResponse<bool>(null, true, onError.toString()));
  }
